...
extension AuthorizedRequest {
  ...
  /// Returns a copy of the current request with state returned by a token refresh.
  func replacing(
    accessToken newAccessToken: IssuanceAccessToken,
    refreshToken newRefreshToken: IssuanceRefreshToken?,
    timeStamp newTimeStamp: TimeInterval,
    dPopNonce newDPopNonce: Nonce?
  ) -> AuthorizedRequest {
    return .init(
      accessToken: newAccessToken,
